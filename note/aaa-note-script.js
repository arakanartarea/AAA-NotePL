...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    expandableList.innerHTML = `<p style="text-align:center;color:red;">Error: ${err.message}</p>`;
    authBtn.innerText = "❌";
  }
});
...
function startListening() {
  ...
  unsubscribe = onSnapshot(collection(db, "AAAnotes"), (snapshot) => {
    ...
    expandableList.innerHTML = `<p style="text-align:center;color:red;">Permission: ${error.message}</p>`;
  });
}
